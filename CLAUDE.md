...
Package path after scaffold: shop/dontforgetshop/app
...
- `ui/theme/Theme.kt` — uses DTFGS naming → apply brand colors
- `ui/theme/Type.kt` — typography → customize if needed
- `SkeletonApp.kt` → already renamed to DTFGSApp
...
- `data/database/SkeletonDatabase.kt` — Room DB (rename class only)
- `data/database/converter/Converters.kt` — type converters
- `data/datastore/OnboardingDataStoreManager.kt` — onboarding flag
...
- `data/repository/OnboardingRepository.kt` — onboarding state
- `data/repository/OrderRepository.kt` — order logic
- `di/*` — Koin DI modules (rename DB class reference only)
...
- `ui/composable/shared/*` — DataBasedContainer, DataEmptyContent
